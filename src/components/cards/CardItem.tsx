...
const CardItem = ({
  ...
  const cardNumberLastFour = String(card.numberId).slice(-4);
    ...
    <li className="w-full flex justify-between items-center border-b border-gray1 md:border-dark1 py-3 md:py-4">
      <div className="flex items-center gap-2.5 md:gap-4">
        ...
        <div className="flex flex-col">
          <p id={`card-${card.numberId}`} className="text-sm md:text-base text-dark1 font-semibold">
      ...
      {showRadio ? (
        <div className="flex items-center">
          <input
            ...
            aria-labelledby={`card-${card.numberId}`}
          />
        </div>
      ) : (
        <button
